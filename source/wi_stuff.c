...
void WI_drawStats(void)
{
    ...
    V_DrawPatch(SP_TIMEX, SP_TIMEY, FB, time0);
...
//			sprintf(name, "CWILV%2.2d", i);
		...
		if (wbs->epsd < 3)
		{
			for (j=0;j<NUMANIMS[wbs->epsd];j++)
			{
				if (wbs->epsd==0)
					...
					a= &epsd2animinfo[j];
				//a = &anims[wbs->epsd][j];  NSPIRE NOT LIKES THIS...
					for (i=0;i<a->nanims;i++)
					{
						...
						{
							// animations
							//sprintf(name, "WIA%i%.2i%.2i", wbs->epsd, j, i);
							//printf("%s \n",name);
							//Bah, Nspire seems to ignore the precision flags in printf/sprintf... let's do it the old fashioned way, then
							if (j<10)
								sprintf(name, "WIA%i0%i0%i",wbs->epsd,j,i);
							else 
								sprintf(name,  "WIA%i%i0%i",wbs->epsd,j,i);
    ...
    // "time"
    time0 = W_CacheLumpName("WITIME", PU_STATIC);   
	...
	//p = W_CacheLumpName("STPB0", PU_STATIC);

	// "1,2,3,4"
	//sprintf(name, "WIBP%d", 1);     
	//bp = W_CacheLumpName("WIBP1", PU_STATIC);
    //}
    ...
    else
    {
		...
		if (wbs->epsd < 3)
		{
			for (j=0;j<NUMANIMS[wbs->epsd];j++)
			{
				if (wbs->epsd != 1 || j != 8)
					{
					//for (i=0;i<anims[wbs->epsd][j].nanims;i++)    NSPIRE DOESN'T SEEM TO LIKE THIS FOR SOME REASON...
